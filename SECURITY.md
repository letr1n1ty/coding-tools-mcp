...
- Workspace-bound cwd.
- Configurable shell environment inheritance with controlled `HOME` and `TMPDIR`.
...
`CODING_TOOLS_MCP_SHELL_ENV_INHERIT=all` broadens compatibility for local toolchains, but still applies the default secret and startup-loader filters. Only combine `--dangerously-skip-all-permissions` with `inherit=all` when the workspace, client, and child processes are trusted enough to receive the full parent environment.
...
Operators may start the server with `--allow-network` to permit network-looking commands while keeping other permission gates active. `--dangerously-skip-all-permissions` auto-grants permission-gated operations for clients that cannot elicit approvals; this mode permits network-looking commands, destructive commands, shell expansion, inline interpreter code, and sensitive env values passed explicitly through `exec_command`; use it only with trusted workspaces and trusted clients. Workspace path boundaries for direct file and patch tools still apply.
